<form class="f1" method="post">
    ...
    <div class="row" style="font-size: 12px;">
        ...
        <div class="col-lg-6">
            ...
            <div class="form-group">
              ...
              <div class="form-group">
                <label for="f1-last-name">Choose Company</label>
                <input class="form-control" {{ (Auth::user()->role == 'Customer Admin' || Auth::user()->role == 'Insurer Admin' || Auth::user()->role == 'Insurer Staff' || Auth::user()->role == 'Others (N/A)') ? 'disabled' : '' }} wire:model="companyquery" type="text" placeholder="Search by Code, Name or Registration No" aria-label="Search by Code, Name or Registration No">
                <div class="list-group">
            ...
            <div class="card-body">
                <ul class="list-group"> 
                    ...
                    @if($customer != null)
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">+255{{ $customer['mobile'] }} | {{ $customer['id_number'] }} ({{ App\Http\Controllers\API\Auth\CustomersController::resolveIdType($customer['id_type']) }})</li>
                    @endif
                </ul>
                <br />
                <ul class="list-group"> 
                ...
                <br />
                <ul class="list-group"> 
                <li class="list-group-item d-flex justify-content-between align-items-center active"><b>Vehicle</b></li>
                @if($vehicle != null)
                    <li class="list-group-item d-flex justify-content-between align-items-center"><b>{{ $vehicle['registration_number'] }}</b> {{ $vehicle['make'] }} {{ $vehicle['model'] }}</li>
                    <li class="list-group-item d-flex justify-content-between align-items-center"><b>Chasis Number</b> {{ $vehicle['chassis_number'] }}</li>
                    <li class="list-group-item d-flex justify-content-between align-items-center"><b>Sitting Capacity</b> {{ $vehicle['sitting_capacity'] }} Seats</li>
                    <li class="list-group-item d-flex justify-content-between align-items-center"><b>Current Owner</b> {{ $vehicle['owner_name'] }}</li>
                @endif
             </ul>
        </div>
        </div>
    ...
    <div class="f1-buttons">
        ...
        <span wire:loading.remove class="checkbox checkbox-dark" style="display: {{ $confirmation_step == 'block' && $is_successfull == false ? '' : 'none' }};">
            ...
            @if($processing_response != null)
            @if($processing_response['responseCode'] == 'SUCCESS')
                <a href="{{ Route('quotation-download', ['id' => $processing_response['quotation']['id']]) }}" class="btn btn-outline-primary btn-next" style="margin-top: auto; display:{{ $is_successfull == true ? '' : 'none' }}" type="button">Print Receipt <i class="icofont icofont-printer"></i></a>
            @endif
            @endif

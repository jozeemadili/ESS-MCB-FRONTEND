<form class="f1" method="post">
    ...
    <div class="row" style="font-size: 12px;">
        ...
        <div class="col-lg-6">
            ...
            <div class="form-group">
                ...
                <select class="form-select" wire:model="category">
                    ...
                    <option value="MISCELLANEOUS AND ACCIDENTS">Miscellaneous and Accidents</option>
            ...
            <div class="form-group">
                ...
                <select class="form-select" wire:model="product_name">
                    @if($products != null)
                    @foreach ($products as $product)
                         @if(count($product->risks)>0)
                             <option value="{{ $product->name }}" {{ $product->status !='Active' ? 'disabled' : '' }}>{{ strtoupper($product->name) }}</option>  
                         @endif
                    @endforeach
                    @endif
                </select>
            ...
            <div class="row">
                <div class="col-lg-7">
                    <div class="form-group">
                        <label class="col-form-label" >Start Date {{ $start_date }}</label>
        ...
        <div class="col-lg-6">
            ...
            <div class="card-body">
                <ul class="list-group"> 
                    <li class="list-group-item d-flex justify-content-between align-items-center active"><b>Customer</b></li>
                    @if($customer != null)
                        <li class="list-group-item d-flex justify-content-between align-items-center"><b>{{ strtoupper($customer['first_name']) }} {{ strtoupper($customer['last_name']) }}</b> {{ $customer['gender'] }}</li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">+255{{ $customer['mobile'] }} | {{ $customer['email'] }}</li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">{{ $customer['id_number'] }} ({{ App\Http\Controllers\API\Auth\CustomersController::resolveIdType($customer['id_type']) }})</li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">Born at {{ date_format(date_create($customer['dob']), 'd M Y') }}</li>
                    @endif
                </ul>
                <br />
                <ul class="list-group"> 
                <li class="list-group-item d-flex justify-content-between align-items-center active"><b>Insurer</b></li>
                @if($company != null)
                     <li class="list-group-item d-flex justify-content-between align-items-center"><b>{{ $company['name'] }}</b></li>
                     <li class="list-group-item d-flex justify-content-between align-items-center">{{ $company['code'] }}</li>
                     <li class="list-group-item d-flex justify-content-between align-items-center">+255{{ $company['phone_number'] }} | {{ $company['email_address'] }}</li>
                     <li class="list-group-item d-flex justify-content-between align-items-center">{{ $company['postal_address'] }}</li>
                     <li class="list-group-item d-flex justify-content-between align-items-center">Reg # {{ $company['registration_number'] }} | TIN # {{ $company['tin'] }}</li>
                @endif
                </ul>
        </div>
        </div>
        ...
        <div class="col-lg-12">
                            ...
                            <div class="card">
                                <div class="media p-10 shadow-lg shadow-showcase">
                                    <div class="radio radio-primary me-3">
                                        <input id="{{ $risk->id }}" {{ $risk->status !='Active' ? 'disabled' : '' }} type="radio" name="risk_id" value="{{ $risk }}" wire:model="risk_id">
                                        <label for="{{ $risk->id }}"></label>
                                    </div>
                                    <div class="media-body">
                                        <h6 class="mt-0 mega-title-badge">{{ $risk->code }} ({{ $risk->premium_rate*100 }}%)<span class="badge badge-{{ $this->getRibbonColor($product->code) }} pull-right digits">{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, $risk, $is_tax_exempted)['total_formated'] }} TZS</span></h6>
            ...
            <div class="ribbon-wrapper card">
                <div class="card-body">
                    <div class="ribbon ribbon-clip ribbon-{{ $this->getRibbonColor($product->code) }}">{{ strtoupper($product->name) }}</div>
                    <div class="row">
                        <pre>{{ print_r($product->risks) }}</pre>
                        @foreach ($product->risks as $risk)
                        <div class="col-lg-3">
                            <div class="card">
                                <div class="media p-10 shadow-lg shadow-showcase">
                                    <div class="radio radio-primary me-3">
                                        <input id="{{ $risk->id }}" type="radio" name="risk_id" value="{{ $risk->id }}">
                                        <label for="{{ $risk->id }}"></label>
                                    </div>
                                    <div class="media-body">
                                        <h6 class="mt-0 mega-title-badge">{{ $risk->code }} ({{ $risk->premium_rate*100 }}%)<span class="badge badge-{{ $this->getRibbonColor($product->code) }} pull-right digits">{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, $risk, $is_tax_exempted)['total'] }} TZS</span></h6>
                                        <p><small>{{ $risk->name }}</small></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-3">
                                <div class="card">
                                    <div class="media p-15">
                                        <div class="radiox radio-primary me-3">
            ...
            <div class="col-lg-6">
                <div class="ribbon-wrapper card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            @if($customer != null && $company != null && $risk_id != null)
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td valign="top" align="left">
                                            <b>{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, json_decode($risk_id), $is_tax_exempted)['cost_formated'] }}</b>
                                        </td>
                                    </tr>
                                    <tr class="pad-left-right-space">
                                        ...
                                        <td></td><td></td>
                                        <td align="left"><b>{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, json_decode($risk_id), $is_tax_exempted)['cost_formated'] }}</b></td>
                                    </tr>
                                    <tr class="pad-left-right-space">
                                        <td align="left">
                                        ...
                                        <td></td><td></td>
                                        <td align="left"><b>{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, json_decode($risk_id), $is_tax_exempted)['tax_formated'] }}</b></td>
                                    </tr>
                                    ...
                                    <tr class="pad-left-right-space">
                                        ...
                                        <td></td><td></td>
                                        <td class="m-b-5" align="left"><b>{{  App\Http\Controllers\API\Pricings\NonMotorEstimationsController::estimate($diff_days, $sum_insured, json_decode($risk_id), $is_tax_exempted)['total_formated'] }}</b></td>
                                    </tr>
                                </tbody>
                            </table>
                            @endif
